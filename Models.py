...
class ConvAE128(nn.Module):
    """
    Convolutional autoencoder made to reconstruct the audio spectrograms generated by the EcoDataTesis dataloader.
    """
    ...
    def __init__(self, num_hiddens: int = 128):
        ...
        # Codificador (Encoder)
        self.encoder = nn.Sequential(
            nn.Conv2d(1, num_hiddens // 8, kernel_size=3, stride=2, padding=1),  # N, 16, 129, 129
            nn.ReLU(),
            nn.Conv2d(num_hiddens // 8, num_hiddens // 4, kernel_size=3, stride=2, padding=1),  # N, 32, 65, 65
            nn.ReLU(),
            nn.Conv2d(num_hiddens // 4, num_hiddens // 2, kernel_size=3, stride=2, padding=1),  # N, 64, 33, 33
            nn.ReLU(),
            nn.Conv2d(num_hiddens // 2, num_hiddens, kernel_size=3, stride=2, padding=1),  # N, 128, 17, 17
            nn.ReLU(),
            nn.Conv2d(num_hiddens, num_hiddens, kernel_size=3, stride=2, padding=1),  # N, 128, 9, 9
            nn.ReLU(),
            nn.Conv2d(num_hiddens, num_hiddens, kernel_size=3, stride=2, padding=1),  # N, 128, 5, 5
            nn.ReLU(),
            nn.Conv2d(num_hiddens, num_hiddens, kernel_size=3, stride=2, padding=1)   # N, 128, 3, 3
        )

        # Decodificador (Decoder)
        self.decoder = nn.Sequential(
            nn.ConvTranspose2d(num_hiddens, num_hiddens, kernel_size=3, stride=2, padding=1),  # N, 128, 5, 5
            nn.ReLU(),
            nn.ConvTranspose2d(num_hiddens, num_hiddens, kernel_size=3, stride=2, padding=1),  # N, 128, 9, 9
            nn.ReLU(),
            nn.ConvTranspose2d(num_hiddens, num_hiddens, kernel_size=3, stride=2, padding=1),  # N, 128, 17, 17
            nn.ReLU(),
            nn.ConvTranspose2d(num_hiddens, num_hiddens // 2, kernel_size=3, stride=2, padding=1),  # N, 64, 33, 33
            nn.ReLU(),
            nn.ConvTranspose2d(num_hiddens // 2, num_hiddens // 4, kernel_size=3, stride=2, padding=1),  # N, 32, 65, 65
            nn.ReLU(),
            nn.ConvTranspose2d(num_hiddens // 4, num_hiddens // 8, kernel_size=3, stride=2, padding=1, output_padding=1),  # N, 16, 129, 129
            nn.ReLU(),
            nn.ConvTranspose2d(num_hiddens // 8, 1, kernel_size=3, stride=2, padding=1, output_padding=0)  # N, 1, 257, 257
        )
...
class ConvAEWithIntermediates(nn.Module):
    def __init__(self, num_hiddens: int = 64):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(1, num_hiddens // 8, kernel_size=8, stride=3, padding=0),
            nn.ReLU(),
            nn.Conv2d(num_hiddens // 8, num_hiddens // 4, kernel_size=8, stride=3, padding=0),
            nn.ReLU(),
            nn.Conv2d(num_hiddens // 4, num_hiddens // 2, kernel_size=4, stride=3, padding=0),
            nn.ReLU(),
            nn.Conv2d(num_hiddens // 2, num_hiddens, kernel_size=2, stride=2, padding=0),
            nn.ReLU()
        )
    ...
    def forward(self, x):
        ...
        for layer in self.decoder:
            x = layer(x)
        reconstructed = x
        return reconstructed, intermediates
...
class ConvAE2(nn.Module):
    ...
    def __init__(self, num_hiddens: int = 64):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(1, num_hiddens // 8, kernel_size=8, stride=3, padding=0),  # N, 256, 127, 8004
            nn.ReLU(),
            nn.Conv2d(num_hiddens // 8, num_hiddens // 8, kernel_size=8, stride=3, padding=0),  # N, 256, 127, 8004
            nn.ReLU(),
            nn.Conv2d(num_hiddens // 8, num_hiddens // 4, kernel_size=8, stride=3, padding=0),  # N, 512, 125,969
            nn.ReLU(),
            nn.Conv2d(num_hiddens // 4, num_hiddens // 4, kernel_size=8, stride=3, padding=0),  # N, 512, 125,969
            nn.ReLU(),
            nn.Conv2d(num_hiddens // 4, num_hiddens // 2, kernel_size=4, stride=3, padding=0),  # N, 512, 125,969
            nn.ReLU(),
            nn.Conv2d(num_hiddens // 2, num_hiddens, kernel_size=2, stride=2, padding=0),  # N, 512, 125,969
        ...
        self.decoder = nn.Sequential(  # This is like go in opposite direction respect the encoder
            nn.ConvTranspose2d(num_hiddens, num_hiddens // 2, kernel_size=2, stride=2, padding=0, output_padding=0),
            nn.ReLU(),
            nn.ConvTranspose2d(num_hiddens//2, num_hiddens // 4, kernel_size=2, stride=2, padding=0, output_padding=0),
            nn.ReLU(),
            nn.ConvTranspose2d(num_hiddens//4, num_hiddens // 4, kernel_size=2, stride=2, padding=0, output_padding=0),
            nn.ReLU(),
            nn.ConvTranspose2d(num_hiddens // 4, num_hiddens // 8, kernel_size=4, stride=3, padding=0,
                               output_padding=0),
            nn.ReLU(),
            nn.ConvTranspose2d(num_hiddens // 8, num_hiddens // 8, kernel_size=2, stride=2, padding=0,
                               output_padding=0),
            nn.ReLU(),
            nn.ConvTranspose2d(num_hiddens // 8, 1, kernel_size=8, stride=3, padding=0, output_padding=0),
            # N, 32, 127,64248
