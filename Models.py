...
class ConvAE(nn.Module):
    ...
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(1, 256, (2, 256), stride=(1, 8), padding=0), # N, 256, 127, 8004
            nn.ReLU(),
            nn.Conv2d(256, 512, (2, 256), stride=(1, 8), padding=0)  # N, 512, 125,969
             )
        self.decoder = nn.Sequential(  # This is like go in opposite direction respect the encoder
            nn.ConvTranspose2d(512, 256, (2, 256), stride=(1, 8), padding=0, output_padding=(0,4)),  # N, 32, 126,8000
            nn.ConvTranspose2d(256, 1, (2, 256), stride=(1, 8), padding=0, output_padding=(0,6)),  # N, 32, 127,64248
            nn.ReLU(),
            nn.Sigmoid()
...
class ConvAE2(nn.Module):
    ...
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            nn.Conv2d(1, 256, (2, 256), stride=(1, 8), padding=0), # N, 256, 128, 4002
            nn.ReLU(),
            nn.Conv2d(256, 512, (2, 256), stride=(1, 8), padding=0)  # N, 512, 127, 235
             )
        self.decoder = nn.Sequential(  # This is like go in opposite direction respect the encoder
            nn.ConvTranspose2d(512, 256, (2, 256), stride=(1, 8), padding=0, output_padding=0),  # N, 32, 7, 7
            nn.ReLU(),
            nn.ConvTranspose2d(256, 1, (2, 256), stride=(1, 8), padding=0, output_padding=0),  # N, 32, 7, 7
            nn.Sigmoid()
...
class VAE2(nn.Module):
    def __init__(self, img_shape, imgChannels=1, num_hiddens : int=64, zDim : int=128):
        ...
        self.num_hiddens = num_hiddens
        self.encConv1 = nn.Conv2d(imgChannels, num_hiddens // 4, 3, 2)
        self.encConv2 = nn.Conv2d(num_hiddens // 4, num_hiddens, 3, 2)

        # stride = [self.encConv1.stride, self.encConv2.stride]
        # padding = [self.encConv1.padding, self.encConv2.padding]
        # kernel = [self.encConv1.kernel_size, self.encConv2.kernel_size]
        # output_padding = [(0, 0), (0, 0)]

        # self.encoded_size, self.decoded_size = cs(self.img_shape, stride=stride,
        #                                           kernel_size=kernel,
        #                                           output_padding=output_padding,
        #                                           dilatation=1)
        self.featureDim = num_hiddens*128*128
        ...
        self.decConv1 = nn.ConvTranspose2d(num_hiddens, num_hiddens // 4, 3, 2)
        self.decConv2 = nn.ConvTranspose2d(num_hiddens // 4, imgChannels, 3, 2)
    ...
    def decoder(self, z):
        ...
        # The generated output is the same size of the original input
        x = F.relu(self.decFC1(z))
        x = x.view(-1, self.num_hiddens, 128, 128)
        x = F.relu(self.decConv1(x))
        x = torch.sigmoid(self.decConv2(x))
        return x
