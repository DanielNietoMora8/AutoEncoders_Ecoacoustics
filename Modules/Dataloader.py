...
class EcoDataTesis(Dataset):
    ...
    """
    EcoDataTesis is the main class used by the dataloader function allowing access to acoustic data, and then train and test
    a deep learning model
    """

    def __init__(self, root_path: str, path_labels: str, path_names: str, ext="wav"):
    ...
    def __getitem__(self, index):
        ...
        f, t, sxx = signal.spectrogram(record, sr)
        sxxt = torch.from_numpy(sxx).float()
        spectrogram = torch.reshape(sxxt, (1, sxxt.shape[0], sxxt.shape[1]))
        return spectrogram, record, sr, features
...
class EcoDataTesisReduced(Dataset):
